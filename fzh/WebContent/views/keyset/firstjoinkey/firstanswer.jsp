...
$(document).ready(function(){
	...
	// 点击取消则恢复
	 $(".form-actions button:last").click(function() {
		 ...
		 //location.href = "${path}/firstKey/firstKeyUI";
		 return false;
	 });
	 ...
	 $("#anform").validate({
		 ...
		 submitHandler : function() {
			 ...
			 if ($("select[name='answertype']").val() == 2) {
				 submitData.imageTextNo = $("#picandtext").val();//选择图文后赋值
			 } else {
				 submitData.reText = $("textarea[name='description']", $form).val();
			 }
			 //alert("提交参数：" );
			 $.post('${path}/firstKey/saveFirstKey', submitData,function(data) {
...
function sltImg(){
	...
	$("#preViewDiv").show();
	/*
	var attr = 'dialogWidth=1000px;dialogHeight=450px;status=no;scroll=yes;dialogTop=150px;dialogLeft=200px;';
	var rs = window.showModalDialog(url, window, attr);
	alert('rs = '+rs[1]);
	if(rs != null && rs !=''){
		$("#preViewDiv").html(rs[0]);
		$("#preViewDiv").show();
		//选择图文后赋值图文Id
		$("#picandtext").val(rs[1]);
	}*/
...
function showImg(){
	flag = true;
	var type = $("#reTypeId").val();
	var imageTextNo = $("#picandtext").val();
	if (type == 2 && imageTextNo != '') {
	    $.post('${path}/imagetextmore/imgPreview/' + imageTextNo, function(data) {
			if (data.success == true) {
				var mainVo = data.imageTextInfo;
				var subVoList = mainVo.subVoList;
				var preItem = "<div class='msg-item multi-msg'>"
								+ "<div class='appmsgItem'>"
								+ "<h4 class='msg-t'><a href='#' class='i-title'>" + mainVo.title + "</a></h4><p class='msg-meta'><span class='msg-date'>" + data.createTime + "</span></p>"
								+ "<div class='cover'><p class='default-tip' style='display:none'>封面图片</p><img src='" + basePath + mainVo.imageUrl + "' class='i-img' style='' /></div>"
								+ "<p class='msg-text'>" + (mainVo.digest==null? "" : mainVo.digest) + "</p>"
								+ "</div>"; // 插入subItem
				var subItem = "";
				if(subVoList != null){
					$.each(subVoList, function(index, obj){
						subItem += "<div class='rel sub-msg-item appmsgItem'><span class='thumb'><img src='"
									  + basePath + obj.imageUrl
									  + "' class='i-img' style='' /></span><h4 class='msg-t'><a href='#' class='i-title'>"
									  + obj.title + "</a></h4></div>";
					}); 
				}
				preItem += subItem;
	...
	<div class="content">
		...
		<div class="span6">
			<form class="form-horizontal" style="display: block;" id="anform">
				<input type="hidden" value="${firstJoinKey.id}" id="id"/>
				<input type="hidden" value="${firstJoinKey.reType}" id="reTypeId"/>
				<fieldset>
					<legend>首次关注回复</legend>
					<!--  
...
-->
					<div class="control-group">
						...
						<div class="controls">
							<select class="span2" id="answertype" name="answertype" value="${firstJoinKey.reType}">
					...
					<div class="control-group" id="textContainer">
						...
						<!-- 引入后未处理之前设置的回复保值问题 -->
						<%-- <jsp:include page="${basePath}/wx/emotion.jsp" /> --%>
						<div class="controls">
							<div class="txtArea">
								...
								<div class="editArea">
									<textarea id="description" name="description"
										style="display: none;"> ${firstJoinKey.reText}</textarea>
		...
		$contentDiv.html(Emotion.replaceEmoji($textarea.html()));
					...
					<div class="control-group" id="picContainer" style="display: none;">
						<label class="control-label" for="imageTextNo">图文资源:</label>
						<div class="controls">
							<input type="hidden" id="picandtext" value="${firstJoinKey.imageTextNo }">
		...
		<p/><div class="span5" id="preViewDiv"></div>
	</div>
	</div>
	</div>
